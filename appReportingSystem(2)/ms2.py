...
class Template:
    ...
    def detailFormatTemplate(kode_laporan):
        try: 
            ...
            cursor.execute(' SELECT c.report_id, c.report_periode, c.report_printAllYN, c.report_judul, c.report_header, c.report_footer, c.report_jumlahTampilan, c.report_deskripsi, a.nama_kolom, a.lokasi, a.format_kolom, a.lebar_kolom, a.format_merge, a.format_rataTengah, a.format_rataKanan, c.report_tujuan FROM m_report c LEFT JOIN m_detailH a ON  c.report_id = a.report_id WHERE c.report_id = "'+kode_laporan+'"  ')
    ...
    def insertDataServer(data):
        if request.method == 'POST' :
            ...
            try:
                ...
                cursor.execute(' INSERT INTO m_server (server_nama, server_loginName, server_password, server_aktifYN, server_host, server_port, server_jenis) VALUES (%s, %s, %s, %s, %s, %s, %s)', (nama_server, login_server, pass_server, aktif_YN, host_server, port_server, jenis_server))
                db.commit()
    ...
    @app.route('/updateDataServer/<data>', methods=['POST', 'GET'])
    def updateDataServer(data):
        if request.method == 'POST' :
            dataServer = json.loads(data)
            ...
            try:
                ...
                cursor.execute(' UPDATE m_server SET server_nama = "'+nama_server+'",\
                                 server_loginName = "'+login_server+'", server_password = "'+pass_server+'",\
                                 server_aktifYN = "'+aktif_YN+'", server_host = "'+host_server+'",\
                                 server_port = "'+port_server+'", server_jenis = "'+jenis_server+'" WHERE server_id = "'+id_server+'" ')
                db.commit()
