...
def previewLaporan(kode_laporan):
	try:
		...
		string_format2 	= workbook.add_format({'num_format': '@','font_size':8,'font_name':'Times New Roman','top':1,'bottom':1})
		integer_format 	= workbook.add_format({'num_format': '0','font_size':8,'font_name':'Times New Roman'})
		integer_format2 = workbook.add_format({'num_format': '0','font_size':8,'font_name':'Times New Roman','top':1,'bottom':1})
		decimal_format 	= workbook.add_format({'num_format': 2,'font_size':8,'font_name':'Times New Roman'})
		decimal_format2 = workbook.add_format({'num_format': 2,'font_size':8,'font_name':'Times New Roman','top':1,'bottom':1})
					...
					try:
						...
						for k in range(lokasiCurr4Len):
							if formatFooter2[k] == 'Integer' and formula2[k] == '':
								...
								worksheet.write(dataSub,lokasiCurr3[k],'=SUM(%s9:%s%s)/2'% (lokasiCurr4[k],lokasiCurr4[k],dataSub),integer_format2)	
							elif formatFooter2[k] == 'Integer' and formula2[k] != '':
								worksheet.write(dataSub,k,'',format_header)
								worksheet.write(dataSub,1,'%s' % (kolomFooter[1]),format_header)
								worksheet.write(dataSub,lokasiCurr3[k],'=SUM(%s%s%s%s%s)'% (formula2[k][0:1],dataSub+1,formula2[k][1:2],formula2[k][2:3],dataSub+1),integer_format2)	
							
							elif formatFooter2[k] == 'Decimal' and formula2[k] == '':
								worksheet.write(dataSub,k,'',format_header)
								worksheet.write(dataSub,1,'%s' % (kolomFooter[1]),format_header)
								worksheet.write(dataSub,lokasiCurr3[k],'=SUM(%s9:%s%s)/2'% (lokasiCurr4[k],lokasiCurr4[k],dataSub),decimal_format2)
							elif formatFooter2[k] == 'Decimal' and formula2[k] != '':
								worksheet.write(dataSub,k,'',format_header)
								worksheet.write(dataSub,1,'%s' % (kolomFooter[1]),format_header)
								worksheet.write(dataSub,lokasiCurr3[k],'=SUM(%s%s%s%s%s)'% (formula2[k][0:1],dataSub+1,formula2[k][1:2],formula2[k][2:3],dataSub+1),decimal_format2)	
								...
								if lokasiCurr3[k] in lokasiCurr:
									worksheet.write(dataSub,lokasiCurr3[k],'=SUM(%s9:%s%s)/2'% (lokasiCurr4[k],lokasiCurr4[k],dataSub),string_format2)	
								else:
									worksheet.write(dataSub,lokasiCurr3[k],'=SUM(%s9:%s%s)'% (lokasiCurr4[k],lokasiCurr4[k],dataSub),string_format2)	
						dataSub = dataSub+1
						countAwal = dataSub+1
